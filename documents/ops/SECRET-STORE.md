...
How embedders wire credential storage for LLM keys and other secrets. The reference shell uses `@qwixl/secret-store` (**monorepo-only** — not on npm). Production hosts should implement the same `SecretStore` interface against an OS-backed vault. Publish to npm later only if embedders need installable dev helpers.
