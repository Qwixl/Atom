...
docname: draft-chapman-a2a-mls-01
category: exp
...
Sessions with more than two members follow MLS group semantics unchanged. Any
member with commit rights, as determined by local policy, MAY add or remove
members. This document places no additional requirements on group membership
policy, but implementations MUST apply the credential binding check of
...
necessary to decrypt past and future epochs.
...
part, obliging implementations of an earlier revision of this document to carry
the media type inside the payload. A receiver that reads only the inner member
...
This document does not establish a registry of purpose values. Purpose
...
A further instance arose in preparing this revision. The reference
...
This revision changes the A2A binding of {{encapsulation}} only. The Governed
Object, its canonical serialisation, signing, governance semantics, identity
construction, credential binding, session establishment, and receiver processing
rules are unchanged, and an implementation conforming to -00 at those layers
remains conforming.

* The three part encodings previously described a part "of kind `data`", which
  was the form taken by A2A version 0.3. Version 1.0 identifies part content
  differently and gives a part its own `mediaType` member. The encodings are
  restated against version 1.0, and {{protocol-version}} now states explicitly
  that members are named as they appear in the wire serialisation, since a
  generated implementation may present them otherwise internally.
* {{media-type-placement}} is new, and specifies that the media type be carried
  both in the part's `mediaType` member and within its `data` object, which
  member governs when only one is present, and that a receiver reject a part
  whose two declarations disagree.
* {{agent-card}} is new. It specifies endpoint declaration through
  `supportedInterfaces`, and Agent Card signatures, including the recommendation
  that `kid` carry the agent's `did:key` so that card verification requires no
  key distribution, and the limits of what a valid signature establishes.
* {{version-compat}} is new, and specifies interoperation with version 0.3
  peers, version determination by the `A2A-Version` header field, and the
  asymmetry of upgrade order in an incrementally upgraded network.
* {{encapsulation}} now recommends that a message list the extension URI in its
  `extensions` member, while requiring that a receiver neither depend on that
  member nor use it in place of the media type.
* Encapsulation test vectors are added, described in {{vectors}}.
* The Implementation Status section records the version 1.0 binding, the
  cross-version interoperation testing, and Agent Card signing. The set of
  requirements specified but not implemented is unchanged from -00.
